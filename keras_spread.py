...
y_train = np.asarray(
  [points[0] - points[1] for points in y_train], dtype=int)
y_validate = np.asarray(
  [points[0] - points[1] for points in y_validate], dtype=int)
y_test = np.asarray(
  [points[0] - points[1] for points in y_test], dtype=int)
...
model = Sequential()
model.add(Bidirectional(GRU(args.rnn_layer_size, return_sequences=True, input_shape=input_shape, dropout=0.1, recurrent_dropout=0.1)))
...
predicted_spreads = np.squeeze(model.predict(x=x_test))
predicted_winners = np.array([spread < 0 for spread in predicted_spreads], dtype=int)
actual_winners = np.array([spread < 0 for spread in y_test], dtype=int)
...
wrong_by_avg = np.mean(np.abs(np.subtract(predicted_spreads, y_test)))
print(
  'The model\'s spread was wrong by an average of {:.2f} points.'.format(
  wrong_by_avg))
wrong_by_med = np.median(np.abs(np.subtract(predicted_spreads, y_test)))
print('The model\'s spread was wrong by a median of {:.2f} points.'.format(
  wrong_by_med))
print('Example games\' actual spread vs predicted:')
for i in range(10):
  print('{}\tvs\t{:.2f}'.format(y_test[i], predicted_spreads[i]))
